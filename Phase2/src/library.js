...
export class Library {
    ...
    async init() {
        try {
            if (this.elements.loading?.style) this.elements.loading.style.display = 'block';
            ...
            if (this.elements.loading?.style) this.elements.loading.style.display = 'none';
        }
    ...
    setupNavPanel() {
        ...
        this.elements.sidebar.appendChild(navPanel);
    ...
    async renderLore() {
        ...
        console.log(`Rendering section: ${sectionId} (${section.title}) in ${this.state.currentMode} mode`);
    ...
    updateBreadcrumbs(section) {
        if (this.elements.breadcrumbs) this.elements.breadcrumbs.innerHTML = '';
        const crumbs = [
            { label: this.state.currentMode.charAt(0).toUpperCase() + this.state.currentMode.slice(1), action: () => this.toggleMode() }
        ];
    ...
    toggleMode() {
        ...
        this.state.currentSectionIds = Object.keys(this.state.index).filter(id => {
            ...
            if (this.state.currentMode === 'canon') {
                return section.source === 'canon';
            } else {
                return section.source.startsWith('proposed-');
            }
        });

        console.log(`Toggled to ${this.state.currentMode} mode. Section IDs:`, this.state.currentSectionIds);
        ...
        this.handleSearch();
        this.updateNavPanel(this.elements.sidebar.querySelector('.lore-nav'));
        this.renderLore();
    }
    ...
    handleSearch() {
        ...
        console.log(`Search results for ${this.state.currentMode} with query "${query}":`, this.state.currentSectionIds);
